...
'Services Received Per Month', 'Continuous', glue('{min(elssp$ServicesReceivedPerMonth, na.rm=TRUE)}-{max(elssp$ServicesReceivedPerMonth, na.rm=TRUE)} services per month (mean (SD): {round(mean(elssp$ServicesReceivedPerMonth, na.rm=TRUE))} ({round(sd(elssp$ServicesReceivedPerMonth, na.rm=TRUE),1)}))'),
...
Our full regression model included all variables except Language Background: 

Vocabulary Delay ~ Gender + Developmental Delay + Health Issues + Prematurity + Laterality + Degree + Amplification + Communication + Meets 1-3-6 + ServicesReceivedPerMonth. 
