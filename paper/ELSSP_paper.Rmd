...
WG_eng_curves <- ggplot()  + 
  geom_line(data=WG_elssp_eng_curves, aes(x=age, y=value,  group=variable, color=variable), alpha= .5)  + 
...
WS_eng_curves <- ggplot()  + 
  geom_line(data=WS_elssp_eng_curves, aes(x=age, y=value,  group=variable, color=variable), alpha= .5)  + 
...
WG_span_curves <- ggplot()  + 
  geom_line(data=WG_elssp_span_curves, aes(x=age, y=value,  group=variable, color=variable), alpha= .5)  + 
...
WS_span_curves <- ggplot()  + 
  geom_line(data=WS_elssp_span_curves, aes(x=age, y=value,  group=variable, color=variable), alpha= .5)  + 
...
cowplot::plot_grid(WG_eng_curves, WS_eng_curves) +
  labs(caption = "Colored lines show vocabulary percentiles from wordbank. black line shows growth curve from Wordbank 50th percentile data. Black triangles show vocabulary scores of individual DHH children.")
cowplot::plot_grid(WG_span_curves, WS_span_curves) +
  labs(caption = "Colored lines show vocabulary percentiles from wordbank. black line shows growth curve from Wordbank 50th percentile data. Black triangles show vocabulary scores of individual DHH children.")

```

To look at the relationship between our predictor variables and CDI scores, we next conducted multiple linear regression, using vocabulary delay as our outcome variable. \footnote{We excluded the non-English child from this section of the analysis due to concerns about comparing her score to the American English CDI norms.}
...
This model accounted for XXX% of the variance in vocabulary delay and included significant main effects of language background (XXX) and developmental delay (XXX). We next performed stepwise model comparison using stepAIC (MASS) to pare down the model. This process selected only the predictors which incrementally improved model fit, measured by Akaike’s Information Criterion (AIC), which considers goodness of fit and model complexity (penalizing models with many predictors). Based on this iterative process, we removed several predictors from the model. 
<!-- eb2ec: 
...
The best fit model was: Age at Diagnosis ~ Health Issues + Language Background + Laterality, with significant main effects of Health Issues and Language Background. This model accounted for `r (summary(diagnosis_aic)$r.squared)*100`% of the variance in age at diagnosis (p=`r lm_pvalue(diagnosis_aic)`). Average age at dianosis for English-speaking children with bilateral hearing loss and no health issues was XXX. Relative to English-speaking families, children from Spanish-speaking families were diagnosed `r beta_output(diagnosis_aic, 3)` months later (p=XXX). Children with health issues were diagnosed `r beta_output(diagnosis_aic, 2)` months later than children without health issues (p=XXX).
...
Amplification devices were more common for children with less hearing (i.e., children with bilateral hearing loss and children with moderate to profound hearing loss). This may be due to the assumption that a hearing aid or cochlear implant will not benefit children with minimal hearing loss [@updike1997], although several studies have found benefits in speech perception and quality of life for amplification for unilateral hearing loss  [@hassepass2013; @priwin2007; @briggs2011] and spoken language vocabulary and grammar for mild hearing loss [@walker2015].
...
All children with mixed hearing loss (n=`r nrow(elssp %>% filter(Etiology=='Mixed'))`) had bilateral hearing loss
