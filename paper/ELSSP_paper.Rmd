...
```{r full-delay-betas, include=F, fig.cap="Beta weights for model of vocabulary delay with all variables included."}
...
This model accounted for significant variance in vocabulary delay (adjusted-R^2^ = `r summary(full_model_delay)$adj.r.squared`, *p* `r lm_pvalue(full_model_delay)`; see Table \@ref(tab:full-delay-betas)). We next performed stepwise model comparison using stepAIC (MASS) to pare down the model. This process selected only the predictors which incrementally improved model fit, measured by Akaike’s Information Criterion (AIC), which considers goodness of fit and model complexity (penalizing models with many predictors). 
