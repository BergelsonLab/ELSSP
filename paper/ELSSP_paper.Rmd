...
kable(research_table,
      caption = "Summary of prior findings examining CDI vocabulary in DHH children.") %>%
  kable_styling(latex_options = c("scale_down")) %>%
  add_footnote(c("+ equals bigger vocab, - equals smaller vocab",
               "N.B. laterality & communication type were not examined in these studies"))
# eb2ec: no reason to have columns that are all NA, i removed them and added the footnote; also changed 'did not study' to NA
# eb2ec: actually, do we need this table? i don't think it adds much, it does make the gender prediction odd since these studies didn't find (or didn't examine) gender effects.
```
...
tribble(
  ...
  glue('WG (n={nrow(elssp %>% filter(CDIversion=="WG"))})'),
  glue(
    '{round(mean((elssp %>% filter(CDIversion=="WG"))$Age), digits=2)} ({round(sd((elssp %>% filter(CDIversion=="WG"))$Age), digits=2)}) months'
  ),
  glue(
    '{round(mean((elssp %>% filter(CDIversion=="WG"))$ComprehensionCDI, na.rm = TRUE), digits=0)} ({round(sd((elssp %>% filter(CDIversion=="WG"))$ComprehensionCDI, na.rm = TRUE), digits=1)}) words'
  ),
  glue(
    '{round(mean((elssp %>% filter(CDIversion=="WG"))$ProductionCDI, na.rm = TRUE), digits=0)} ({round(sd((elssp %>% filter(CDIversion=="WG"))$ProductionCDI, na.rm = TRUE),digits=1)}) words'
  ),
  glue(
    '{round(((nrow((elssp %>% filter(CDIversion=="WG")) %>% filter(DevelopmentalConcerns=="yes")))/(nrow((elssp %>% filter(CDIversion=="WG")))))*100, digits=1)}%'
  ),
  glue('WS (n={nrow((elssp %>% filter(CDIversion=="WS")))})'),
  glue(
    '{round(mean((elssp %>% filter(CDIversion=="WS"))$Age), digits=2)} ({round( sd((elssp %>% filter(CDIversion=="WS"))$Age), digits=2)}) months'
  ),
  'NA',
  glue(
    '{round(mean((elssp %>% filter(CDIversion=="WS"))$ProductionCDI, na.rm = TRUE), digits=0)} ({round(sd((elssp %>% filter(CDIversion=="WS"))$ProductionCDI, na.rm = TRUE), digits=1)}) words'
  ),
  glue(
    '{round(((nrow((elssp %>% filter(CDIversion=="WS")) %>% filter(DevelopmentalConcerns=="yes")))/(nrow((elssp %>% filter(CDIversion=="WS")))))*100, digits=1)}%'
  )
) %>%
  ...
  kable_styling(latex_options = c("scale_down"))
# eb2ec: please sort out how to make this kind of table with less redundant/unreadable coding( like the 1000 instances of {round(mean((elssp %>% filter(CDIversion) before the final version is shared.
```
...
kable(
  tribble(
    ...
    glue(
      '{round((nrow(elssp%>%filter(meets13=="yes"))/(nrow(elssp %>% filter(meets13=="yes"|meets13=="no"))))*100,2)}%'
    ),
    'Average Age Diagnosis (SD)',
    glue(
      '{round(mean(elssp$IdentificationOfHLMonths, na.rm = TRUE), digits=2)} ({round(sd(elssp$IdentificationOfHLMonths, na.rm = TRUE), digits=2)}) months'
    ),
    'Intervention by 6 months',
    glue(
      '{round((nrow(elssp%>%filter(meets6=="yes"))/(nrow(elssp %>% filter(meets6=="yes"|meets6=="no"))))*100,2)}%'
    ),
    'Average Age Intervention (SD)',
    glue(
      '{round(mean(elssp$AgeStartedServices, na.rm = TRUE), digits=2)} ({round(sd(elssp$AgeStartedServices, na.rm = TRUE), digits=2)}) months'
    ),
    'Meets 1-3-6',
    glue(
      '{round((nrow(elssp%>%filter(Meets136=="yes"))/(nrow(elssp %>% filter(Meets136=="yes"|Meets136=="no"))))*100,2)}%'
    )
  ),
...
#eb2ec: this table is showing up weird (and also same comment as before about the redundancy/illegibility of the code). i think you probably want % and age as different columns with diag and intervention as your 2 rows, and maybe the % meetings 136 just in the caption? but actually, as i note below, i think you can nix this table altogether
```
...
tribble(
  ...
  'Continuous',
  glue(
    '{min(elssp$Age)}-{round(max(elssp$Age), digits=0)} months (mean (SD): {round(mean(elssp$Age))} ({round(sd(elssp$Age),1)}))'
  ),
  'Age at Amplification',
  'Continuous',
  glue(
    '{min(elssp$AgeAmplification, na.rm=TRUE)}-{max(elssp$AgeAmplification, na.rm=TRUE)} months (mean (SD): {round(mean(elssp$AgeAmplification, na.rm=TRUE))} ({round(sd(elssp$AgeAmplification, na.rm=TRUE),1)}))'
  ),
  'Age at Diagnosis',
  'Continuous',
  glue(
    '{min(elssp$IdentificationOfHLMonths, na.rm=TRUE)}-{max(elssp$IdentificationOfHLMonths, na.rm=TRUE)} months (mean (SD): {round(mean(elssp$IdentificationOfHLMonths, na.rm=TRUE))} ({round(sd(elssp$IdentificationOfHLMonths, na.rm=TRUE),1)}))'
  ),
  'Age at Implantation',
  'Continuous',
  glue(
    '{min(elssp$AgeImplantation, na.rm=TRUE)}-{max(elssp$AgeImplantation, na.rm=TRUE)} months (mean (SD): {round(mean(elssp$AgeImplantation, na.rm=TRUE))} ({round(sd(elssp$AgeImplantation, na.rm=TRUE),1)}))'
  ),
  'Age at Intervention',
  'Continuous',
  glue(
    '{min(elssp$AgeStartedServices, na.rm=TRUE)}-{max(elssp$AgeStartedServices, na.rm=TRUE)} months (mean (SD): {round(mean(elssp$AgeStartedServices, na.rm=TRUE))} ({round(sd(elssp$AgeStartedServices, na.rm=TRUE),1)}))'
  ),
  'Amplification',
  'Categorical',
  ...
  'Continuous',
  glue(
    '{min(elssp$HLworse, na.rm=TRUE)}-{max(elssp$HLworse, na.rm=TRUE)} dB HL (mean (SD): {round(mean(elssp$HLworse, na.rm=TRUE))} ({round(sd(elssp$HLworse, na.rm=TRUE))}))'
  ),
  'Developmental Delay',
  'Categorical',
  'Yes / No',
  ...
  'Continuous',
  glue(
    '{min(elssp$ServicesReceivedPerMonth, na.rm=TRUE)}-{max(elssp$ServicesReceivedPerMonth, na.rm=TRUE)} services per month (mean (SD): {round(mean(elssp$ServicesReceivedPerMonth, na.rm=TRUE))} ({round(sd(elssp$ServicesReceivedPerMonth, na.rm=TRUE),1)}))'
  ),
  'Etiology',
  'Categorical',
  ...
  'Continuous',
  glue(
    '{min(elssp$ProductionCDI, na.rm=TRUE)}-{max(elssp$ProductionCDI, na.rm=TRUE)} words (mean (SD): {round(mean(elssp$ProductionCDI, na.rm=TRUE))} ({round(sd(elssp$ProductionCDI, na.rm=TRUE),1)}))'
  ),
) %>% kable(caption = "Detailed information about the variables studied. For categorical variables, levels are described. For continuous variables, range, mean, and standard deviation are provided.", digits =
              2) %>%
  kable_styling(latex_options = "scale_down") 
#eb2ec:  same comment as before about the redundancy/illegibility of the code)
```
...
#eb2ec: let's get all english and spanish curves in a single 4panel figure that is less tall; i'd also use open circles so overlap is clearer
...
```{r full-delay-betas}
#eb2ec: all of your tables with model results have way too many sig digits and the variable names need to be interpretable to readers 

#eb2ec: i actually don't think you need the full model table at all
...
AmplificationCI= "Amplification (Cochlear Implant)") + 
  ...
  theme(legend.position = "none")
#eb2ec: let's get all 3 figs like this into a single figure and let's squash them so they're less tall (same for the curves). figures should only be as wide and tall as necessary for clarity and no bigger.
# eb2ec:are the error bars on age truly so tiny as to be invisible? see my other note about age above; is it possible you have age in a unit other than months or something?
```
...
```{r delay-vif}
#eb2ec: you do not need this or ANY VIF tables in this paper
...
<!-- eb2ec: this last sentence doesn't make sense to me and its weird to switch to weeks. it sounds like you're saying that each month of age meant you were 4 more months behind the hearing 50%th percentile--is that right?? that doesn't seem right. -->
...
#eb2ec: why use standard deviation here and standard error in the other one?
...
<!-- eb2ec: maybe start this para with "We predicted gender, ... would predict vocab delay." -->
...
<!-- eb2ec: i don't think the preceding paragraph is doing anything for us, i'd cut it (commented it out for now) -->
...
<!-- eb2ec: here and in the results, it seems weird to say 4.02 weeks instead of 1 month since your'e generally talking in months -->
...
<!-- eb2ec: some of your ref's are rendering as n.d. for the date -->
...
<!-- eb2ec: i took a stab at conclusion rewrite -->
\pagebreak
# References
