...
CDI_table <- tribble(
  ...
  glue('WG (n={nrow(wg_elssp)})'), glue('{round(mean(wg_elssp$Age), digits=2)} ({round(sd(wg_elssp$Age), digits=2)}) months'), glue('{round(mean(wg_elssp$ComprehensionCDI, na.rm = TRUE), digits=0)} ({round(sd(wg_elssp$ComprehensionCDI, na.rm = TRUE), digits=1)}) words'), glue('{round(mean(wg_elssp$ProductionCDI, na.rm = TRUE), digits=0)} ({round(sd(wg_elssp$ProductionCDI, na.rm = TRUE),digits=1)}) words'), glue('{round(((nrow(wg_elssp %>% filter(DevelopmentalConcerns=="1")))/(nrow(wg_elssp)))*100, digits=2)}%'),
  glue('WS (n={nrow(ws_elssp)})'), glue('{round(mean(ws_elssp$Age), digits=2)} ({round( sd(ws_elssp$Age), digits=2)}) months'), 'NA', glue('{round(mean(ws_elssp$ProductionCDI, na.rm = TRUE), digits=0)} ({round(sd(ws_elssp$ProductionCDI, na.rm = TRUE), digits=1)}) words'), glue('{round(((nrow(ws_elssp %>% filter(DevelopmentalConcerns=="1")))/(nrow(ws_elssp)))*100, digits=2)}%'))
...
For categorical-categorical relationships, we used chi square tests; for continuous-categorical tests, we used mann-whitney U tests (2 levels for categorical variable) or kruskal-wallis tests (>2 levels for categorical variable; for continuous-continuous relationships, we used Of the fifty-five combinations of variables, p < .05 for sixteen, and seven survived bonferroni correction (p < `r format.pval(.05/55, digits = 2)`). The full set of comparisons is shown in figure XXX.
...
ggplot(chi_sq_all, aes(y=Var1, x=Var2)) +
...
From this analysis, we found that children born premature were more likely to also have health issues `r chisq_output(Prematurity_HealthIssues)`. Children with conductive hearing loss were more likely to have unilateral hearing loss `r chisq_output(Laterality_Etiology)`. Children with unilateral hearing loss were unlikely to receive a cochlear implant and more likely to use no amplification `r chisq_output(Laterality_Amplification)`. Children with more severe hearing loss were more likely to use a cochlear implant than children with milder hearing loss (H(`r Degree_Amplification$parameter`)=`r Degree_Amplification$statistic`, p=`r Degree_Amplification$p.value`). Children with developmental delays received more services per month than typically developing DHH children (H(`r Services_DevConcerns$parameter`)=`r Services_DevConcerns$statistic`, p=`r Services_DevConcerns$p.value`)and were more likely to use total communication `r chisq_output(DevConcerns_Communication)`. Children who used total communication received more services per month than children using spoken language (H(`r Services_Communication$parameter`)=`r Services_Communication$statistic`, p=`r Services_Communication$p.value`).
