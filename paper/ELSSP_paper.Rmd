...
##Methods
``` {r demo-data}
num_subs = nrows(full_elssp)
num_repeat_subs = nrows(elssp %>% filter(VisitNumber == 2))
num_girls = nrows(full_elssp %>% filter(Gender == 'female'))
num_boys = nrows(full_elssp %>% filter(Gender == 'male'))
min_age = min(full_elssp$Age)
max_age = max(full_elssp$Age)
mean_age = mean(full_elssp$Age)
sd_age = sd(full_elssp$Age)
num_too_old_wg = nrows(wg_elssp %>% filter(Age>18)))
num_spanish_cdi = nrows(full_elssp %>% filter(PrimaryLanguage == 'Spanish))
```
...
The best fit model (R2=`r summary(diagnosis_aic)$r.squared` , p=`r lm_pvalue(diagnosis_aic)` )included health issues (beta weight = `r summary(diagnosis_aic)$coefficients[2,4]`, p = `r summary(diagnosis_aic)$coefficients[2,1]`) and language background (beta weight = `r summary(diagnosis_aic)$coefficients[3,4]`, p = `r summary(diagnosis_aic)$coefficients[3,1]`).
...
The best fit model (R2=`r summary(services_aic)$r.squared` , p=`r lm_pvalue(services_aic)`) included prematurity (beta weight = `r summary(diagnosis_aic)$coefficients[4,4]`, p = `r summary(diagnosis_aic)$coefficients[4,1]`), laterality (beta weight = `r summary(diagnosis_aic)$coefficients[3,4]`, p = `r summary(diagnosis_aic)$coefficients[3,1]`), degree of hearing loss (beta weight = `r summary(diagnosis_aic)$coefficients[2,4]`, p = `r summary(diagnosis_aic)$coefficients[2,1]`), and age at diagnosis (beta weight = `r summary(diagnosis_aic)$coefficients[5,4]`, p = `r summary(diagnosis_aic)$coefficients[5,1]`). 
