...
Given that we ran XX Chi-square tests, Bonferroni-corrected alpha for this set of analyses was p < 0.0007. Of these `r nrow(chi_sq_all)` combinations of variables, p < .05 for `r nrow(chi_sq_all %>% filter(p.value<.05))`, and `r nrow(chi_sq_all %>% filter(p.value<.000757))` survived Bonferroni correction. We are only discussing the latter below, but the full set of results can be found in table XXX.
...
# that plot_elssp_df function is way more complicated than you need it to be (as evidenced by your not being sure why the graph is printing multiple times:)). simplify the code. get help from char/shan/fed for this.
```
...
```
# Discussion
<!-- eb: time to outline this section -->
