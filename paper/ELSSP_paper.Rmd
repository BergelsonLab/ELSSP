...
tribble(
  ...
  glue('WG (n={nrow(elssp %>% filter(CDIversion=="WG"))})'), glue('{round(mean((elssp %>% filter(CDIversion=="WG"))$Age), digits=2)} ({round(sd((elssp %>% filter(CDIversion=="WG"))$Age), digits=2)}) months'), glue('{round(mean((elssp %>% filter(CDIversion=="WG"))$ComprehensionCDI, na.rm = TRUE), digits=0)} ({round(sd((elssp %>% filter(CDIversion=="WG"))$ComprehensionCDI, na.rm = TRUE), digits=1)}) words'), glue('{round(mean((elssp %>% filter(CDIversion=="WG"))$ProductionCDI, na.rm = TRUE), digits=0)} ({round(sd((elssp %>% filter(CDIversion=="WG"))$ProductionCDI, na.rm = TRUE),digits=1)}) words'), glue('{round(((nrow((elssp %>% filter(CDIversion=="WG")) %>% filter(DevelopmentalConcerns=="yes")))/(nrow((elssp %>% filter(CDIversion=="WG")))))*100, digits=1)}%'),
  glue('WS (n={nrow((elssp %>% filter(CDIversion=="WS")))})'), glue('{round(mean((elssp %>% filter(CDIversion=="WS"))$Age), digits=2)} ({round( sd((elssp %>% filter(CDIversion=="WS"))$Age), digits=2)}) months'), 'NA', glue('{round(mean((elssp %>% filter(CDIversion=="WS"))$ProductionCDI, na.rm = TRUE), digits=0)} ({round(sd((elssp %>% filter(CDIversion=="WS"))$ProductionCDI, na.rm = TRUE), digits=1)}) words'), glue('{round(((nrow((elssp %>% filter(CDIversion=="WS")) %>% filter(DevelopmentalConcerns=="yes")))/(nrow((elssp %>% filter(CDIversion=="WS")))))*100, digits=1)}%')) %>%
kable(caption = "CDI details: For each version, average age, comprehension score, production score of participants in our sample (with standard deviations in parentheses). Final column shows the percentage of children administered the version who were diagnosed with developmental delays.") %>%
  kable_styling(latex_options = c("scale_down"))
```
...
elssp %>% 
  ...
  kable(caption = glue::glue("Audiological Characteristics of the Sample: First two columns describe laterality and amplification type (cochlear implant, hearing aid, or none). Next two columns show mean decibels of hearing loss in better/worse ear for the subset of the population given that combination of laterality and amplification. Last two columns describe age (in months) at amplification or cochlear implantation (if applicable) for the laterality-amplification subset.")) %>%
...
'Meets 1-3-6', glue('{round((nrow(elssp%>%filter(Meets136=="yes"))/(nrow(elssp %>% filter(Meets136=="yes"|Meets136=="no"))))*100,2)}%')
      ...
      caption = "Meets 1-3-6 Information: Percent of the sample meeting diagnosis and/or intervention guidelines, and average age diagnosis/intervention was provided.") %>%
...
) %>% kable(caption = "Detailed information about the variables studied. For categorical variables, levels are described. For continuous variables, range, mean, and standard deviation are provided.", digits=2) %>%
...
```{r english-curves, fig.cap="Lines show the growth curves created from Wordbank American English 50th percentile data. Left panel shows Words & Gestures; right panel shows Words & Sentences. Dots represent vocabulary scores of individual DHH children in the sample."}
cowplot::plot_grid(WG_eng_curves, WS_eng_curves) 
```

```{r spanish-curves, fig.cap="Lines show the growth curves created from Wordbank Mexican Spanish 50th percentile data. Left panel shows Words & Gestures; right panel shows Words & Sentences. Dots represent vocabulary scores of individual DHH children in the sample."}
...
```{r full-delay-betas, include=FALSE}
...
full_model_delay %>% 
  tidy() %>% 
  kable(caption = "Unstandardized beta weights (months of vocabulary delay) for model of vocabulary delay with all variables included: Vocabulary Delay ~ Gender + Developmental Delay + Health Issues + Premature Birth + Laterality + Degree + Amplification + Communication + Meets 1-3-6 + Services Received Per Month + Language Background.")  %>%
  kable_styling()
```

```{r delay-AIC, include=F}
#Started with full model (in code chunk above)
...
# filtered_AIC_model_delay <- lm(formula = diff_age_from_expected ~ DevelopmentalConcerns + Laterality + Amplification + Age, data = (elssp_curves %>% filter(PrimaryLanguage != "Hindi" & Meets136 != "")) %>% droplevels())

#Because Degree did not improve model fit, the Degree term and filter were removed manually from the model, so that participants without Degree information were not excluded.
full_model_delay_nodegree <- lm(diff_age_from_expected ~ Gender + DevelopmentalConcerns + IsPremature + HealthIssues + Laterality + Amplification + Communication + Meets136 + PrimaryLanguage +Age, data = (elssp_curves %>% filter(PrimaryLanguage!='Hindi' & Meets136!='')) %>% droplevels())
...
This model accounted for significant variance in vocabulary delay (adjusted-R^2^ = `r summary(full_model_delay)$adj.r.squared`, *p* `r lm_pvalue(full_model_delay)`; see Table \@ref(tab:full-delay-betas)). We next performed stepwise model comparison using stepAIC (MASS) to pare down the model. This process selects only the predictors which incrementally improved model fit, measured by Akaike’s Information Criterion (AIC), which considers goodness of fit and model complexity (penalizing models with many predictors). We started model selection with the full model, as described above. We then filtered out cases where Meets 1-3-6 or Degree was unknown; stepwise AIC selection won't run with missing values in those columns due to differences in the number of cases. Because Degree did not improve model fit, the Degree term was removed manually from the model, so that participants without Degree information were not excluded. Stepwise selection with AIC was performed again without Degree\footnote{When AIC selection was run excluding the participants without Degree information, the model also included a non-significant main effect of Developmental Delay. ANOVA revealed that including a Developmental Delay term did not significantly improve model fit when including the `r nrow(elssp %>% filter(HLworse=='NA'))` participants without Degree information.}. 

```{r delay-beta-plot, fig.cap="Unstandardized beta weights (months of vocabulary delay) for the model of vocabulary delay selected by AIC: Vocabulary Delay ~ Age + Laterality + Amplification."}
...
AIC_model_delay %>% 
  tidy() %>% 
  kable(caption = "Unstandardized beta weights (months of vocabulary delay) for the model of vocabulary delay selected by AIC: Vocabulary Delay ~ Age + Laterality + Amplification.") %>%
  kable_styling()
```

```{r delay-vif}
kable((vif(AIC_model_delay))[,1:2], caption = "Generalized Variance Inflation Factor (GVIF) for the model of vocabulary delay selected by AIC: Vocabulary Delay ~ Age + Laterality + Amplification.") %>% 
  kable_styling() 
...
```{r meets136-timeline, fig.cap="Violin plots for age at diagnosis, intervention, amplification, and cochlear implantation. Each dot represents the age that one child received the clinical service. Blacks dot and whiskers show means and standard deviations."}
...
```{r delay-violins, fig.cap="Violin plots showing vocabulary delay for children who meet 1-3-6 guidelines for diagnosis/intervention (top violin) and children who do not (bottom violin). Each dot represents one child in the sample. Blacks dot and whiskers show means and standard error."}
...
```{r diagnosis-beta-plot, fig.cap="Unstandardized beta weights (months) for the model of age at diagnosis selected by AIC: Vocabulary Delay ~ Health Issues + Language Background + Laterality."}
...
diagnosis_aic %>% 
  tidy() %>% 
  kable(caption = "Unstandardized beta coefficients (months) for the model of age at diagnosis selected by AIC: Vocabulary Delay ~ Health Issues + Language Background + Laterality.") %>%
  kable_styling()
```
...
```{r intervention-beta-plot, fig.cap="Unstandardized beta coefficients (months) for the model of age at intervention selected by AIC: Vocabulary Delay ~ Degree + Prematurity + Language Background + Age at Diagnosis."}
...
services_aic %>% 
  tidy() %>%
  kable(caption = "Unstandardized beta coefficients (months) for the model of age at intervention selected by AIC: Vocabulary Delay ~ Degree + Prematurity + Language Background + Age at Diagnosis.") %>% 
  kable_styling()
```
...
<!-- ec2ec answer this question immediately -->
```{r disc1-values}
...
A final point regarding 1-3-6 attainment: this sample is composed of children receiving birth-to-3 services. An estimated 67% of children with hearing loss enroll in early intervention services [@cdc2018]. While this represents a tremendous step forward in prompt early intervention services relative to just a few decades prior, early intervention may not be early enough. Less than `r round((nrow(elssp%>%filter(meets6=="yes"))/(nrow(elssp %>% filter(meets6=="yes"|meets6=="no"))))*100)`% of our sample of children in early intervention meet the 6-month EHDI benchmark. Furthermore, an unknown fraction of the DHH population in North Carolina aren’t included in this analysis because they have not been enrolled in services by 36 months. The AAP estimates that almost 36% of infants who do not pass a newborn hearing screening are lost to follow-up. Assuming that the population of children in early intervention only represents two thirds of the population with hearing loss, our data suggest that the actual proportion of DHH children who receive intervention by the EHDI-recommended 6 months may be closer to `r round((nrow(elssp%>%filter(meets6=="yes"))/(nrow(elssp %>% filter(meets6=="yes"|meets6=="no"))*3/2))*100)`%. These children may not receive clinical support until school-age or later. 
