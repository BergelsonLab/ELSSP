...
Before we explore how these variables may be related to vocabulary, we would like to describe the variables’ relationships to each other. Our goal in doing so is to demonstrate that many of these characteristics are not distributed randomly throughout the population. We approach this with bonferroni-corrected chi-square tests between each of our variables (gender, laterality, health issues, developmental delays, prematurity, language background, 1-3-6, degree of hearing loss (binned into mild, moderate, severe/profound), etiology, services received per month (binned into 0-3, 4-10, and >10), and amplification). 

Bonferroni-corrected alpha for this set of analyses was p < 0.0007. Of the `r nrow(chi_sq_all)` combinations of variables, p < .05 for `r nrow(chi_sq_all %>% filter(p.value<.05))`, and `r nrow(chi_sq_all %>% filter(p.value<.000757))` survived bonferroni correction. We are only discussing the results of tests that survived bonferroni correction, but the full set of results can be found in table XXX.
...
This model accounted for roughly `r summary(AIC_model_delay)$r.squared`% of the variance in children’s vocabulary delay. 
...
In addition to the model, we ran bonferroni-corrected zero-order tests of each predictor on vocabulary delay. Because vocabulary delay deviated significantly from a normal distribution (p<.001 on Shapiro-Wilk normality test), we used non-parametric tests for this set of analyses. For 2-level categorical variables (i.e., Gender, Developmental Concerns, Health Issues, Prematurity, Laterality, Communication, and 1-3-6), we used Wilcoxen Mann-Whitney tests; for >2-level categorical variables (i.e., Etiology, Amplification), we used kruskal-wallis tests; and for continuous variables (i.e., Services Received Per Month, Degree of Hearing Loss), we used Spearman's correlations. Bonferroni-corrected alpha for this set of analyses was p<0.0045. None of the tests survived Bonferroni-correction. We present the results in Table XXX.

```{r delay-analysis, include=F}
wilcox.test(data = full_elssp, diff_age_from_expected ~ Gender) #sig
wilcox.test(data = full_elssp, diff_age_from_expected ~ DevelopmentalConcerns) #sig
wilcox.test(data = full_elssp, diff_age_from_expected ~ HealthIssues) #ns
wilcox.test(data = full_elssp, diff_age_from_expected ~ IsPremature) #ns
wilcox.test(data = (full_elssp %>% filter(Laterality == 'Bilateral'|Laterality == 'Unilateral')), diff_age_from_expected ~ Laterality) #ns
wilcox.test(data = (full_elssp %>% filter(Meets136 == 'yes' | Meets136 == 'no')), diff_age_from_expected ~ Meets136) #ns
wilcox.test(data = (full_elssp %>% filter(Communication == 'spoken' | Communication == 'total communication')), diff_age_from_expected ~ Communication) #ns

kruskal.test(data = full_elssp, diff_age_from_expected ~ Amplification) #sig
kruskal.test(data = full_elssp, diff_age_from_expected ~ Etiology) #ns

cor.test(full_elssp$diff_age_from_expected, full_elssp$HLworse, method = 'spearman') #ns
cor.test(full_elssp$diff_age_from_expected, full_elssp$ServicesReceivedPerMonth, method = 'spearman') #ns
```
...
```
Lastly, we looked at the ages at which children received diagnosis and intervention, and how this mapped onto the 1-3-6 guidelines. Overall, `r round(perc_136)`% of our sample met 1-3-6 guidelines for early diagnosis and intervention. 
...
stepAIC(lm(AgeStartedServices ~ Gender + 
             ...
             anycomorbid, 
           (data=full_elssp %>% filter(HLworse!="NA"& Etiology!='')))) 
