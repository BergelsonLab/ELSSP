...
<!-- eb2ec: (and maybe combine the last two sentences to condense?) -->
...
full_model_delay2 <- lm(diff_age_from_expected ~ Gender + DevelopmentalConcerns + IsPremature + HealthIssues + Laterality + Amplification + Communication + Meets136 + PrimaryLanguage +Age, data = (elssp_curves %>% filter(PrimaryLanguage!='Hindi' & Meets136!='')) %>% droplevels())
...
```{r delay-betas, results="asis", fig.cap="Coefficients for the final model.", fig.height = 3, fig.width = 2}
...
```{r delay-vif, fig.cap="Generalized Variance Inflation Factor for the final model."}
