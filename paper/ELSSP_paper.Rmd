...
AIC_model_delay %>% 
  ...
  kable(caption = "Unstandardized beta weights (months of vocabulary delay) for the model of vocabulary delay selected by AIC.", digits = 2) %>%
  kable_styling()
```
...
# ec2eb: standard error looks really tiny. (aesthetically) should we switch to SD or is this fine?
gg_SE <- function(x) {
   m <- mean(x)
   ymin <- m-se(x)
   ymax <- m+se(x)
...
diagnosis_aic %>% 
  ...
  kable(caption = "Unstandardized beta coefficients (months) for the model of age at diagnosis selected by AIC.", digits=2) %>%
  kable_styling()
```
...
services_aic %>% 
  tidy() %>%
    mutate(term=replace(term, term=="HLworse", "Degree of Hearing Loss"),
           ...
           term=replace(term, term=="Monolingual_Englishyes", "Language Background (English)")) %>%
  kable(caption = "Unstandardized beta coefficients (months) for the model of age at intervention selected by AIC.", digits = 2) %>% 
  kable_styling()
```
...
```{r beta-plots, fig.cap="Unstandardized coefficients (measured in months) with 95% confidence intervals for the models selected by AIC for: (A) vocabulary delay, (B) age at diagnosis, (C) age at intervention.", fig.dim=c(8,4)}
