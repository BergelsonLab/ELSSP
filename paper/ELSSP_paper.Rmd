...
Independently of hearing status, prematurity is linked to increased risk of language delay and disorder [@rechia2016; @vannoort-vanderspek2012; @carter2017; @vohr2014; @cusson2003; @barre2011]. Unfortunately, research on language development in premature DHH children is scant [@vohr2016], so it remains unclear how hearing loss and prematurity may interact within spoken language skills. One study of premature infants finds that auditory brainstem response during newborn hearing screening (NBHS) predicts language performance on the PLS-4 at age 3 [@amin2014], suggesting a link between prematurity, hearing loss, and language development in early childhood, though further research is needed in this domain. In extremely premature DHH children, incidence of additional disabilities may be as high as 73% [@robertson2009]. Indeed, pre-term infants with comorbidities have been found to be more likely to also have hearing loss than those without comorbidities [@schmidt2003], further complicating language development for this population.
...
<!-- eb2ec: i think there's something wrong with how the ref's are rendering in the above para-have a look. Also: specify whether NC includes mild in the next sentence (and maybe combine the last two sentences to condense?) -->
...
```{r relationships-plot, fig.width=12, fig.height=9.5, fig.cap="Results of chi-square tests between variables. For tests that survived Bonferroni correction, effect size (Cramer's V) is given."}
# eb2ec: no it is not!
...
```{r english-curves, fig.cap="Growth curve from Wordbank American English 50th percentile data. Black triangles show vocabulary scores of individual DHH children."}
cowplot::plot_grid(WG_eng_curves, WS_eng_curves) 
```

```{r spanish-curves, fig.cap="Growth curve from Wordbank Spanish (Mexican) 50th percentile data. Black triangles show vocabulary scores of individual DHH children."}
...
#eb2ac: make sure  your code is clean and clear and anything in it needs to be there or is understandable to someone else or future you, and that the text description provides all necessary information so that the reader understands how the model selection worked
...
```{r full-delay-betas, fig.cap="Coefficients for the full model."}
dwplot(full_model_delay)  %>%
  relabel_predictors(Gendermale = "Gender (male)",
                     DevelopmentalConcernsyes = "Developmental Delay (yes)",
                     IsPrematureyes = "Prematurity (yes)",
                     HealthIssuesyes = "Health Issues (yes)",
                     LateralityUnilateral = "Laterality (Unilateral)",
AmplificationHA= "Amplification (Hearing Aid)",
Amplificationnone= "Amplification (none)",
Communicationspoken = "Communication (spoken)",
"Communicationtotal communication" = "Communication (total communication)",
Meets136yes= "1-3-6 (yes)",
PrimaryLanguageSpanish = "Language Background (Spanish)",
HLworse = "Degree (worse ear)") +
  geom_vline(xintercept=0, linetype = 'dashed') + 
  theme_classic() + 
  theme(legend.position="none")
options(repr.plot.width=6, repr.plot.height=4) 

#why does total communication have its own line? what's the reference level here?

# eb2ec: not sure who left the note above (you to you?) but i was also confused about why amplification shows up twice, leading me to wonder how you'd set up the reference levels for amplification, and also, no caption for this figure?
```

This model accounted for significant variance in vocabulary delay (adjusted-R^2^ = `r summary(full_model_delay)$adj.r.squared`, *p* `r lm_pvalue(full_model_delay)`; see Figure \@ref(fig:full-delay-betas)). We next performed stepwise model comparison using stepAIC (MASS) to pare down the model. This process selected only the predictors which incrementally improved model fit, measured by Akaike’s Information Criterion (AIC), which considers goodness of fit and model complexity (penalizing models with many predictors). 

<!-- ec2eb: does it make any sense to mention stats for the full model? should i cut this?//eb2ec: fine to include in text but i woud probably only have the figur eof the final one -->

```{r delay-betas, results="asis", fig.cap="Coefficients for the final model."}
...
```{r meets136-timeline, fig.cap="Timeline for diagnosis/intervention/etc."}
...
```{r delay-violins, fig.cap="Months delay for children who meet / don't meet 1-3-6 guidelines."}
...
<!-- eb2ec: the pvalues are all well and good but what's the actual mean difference in month's delay here? -->
...
```{r diagnosis-betas, results="asis", fig.cap="Coefficients for diagnosis model."}
...
We repeated this model selection process for age at intervention. In addition to the variables used to fit the intervention model, we included age at diagnosis. The best fit model was: Age at Intervention ~ Premature Birth + Degree + Age at Diagnosis + Language Background (R^2^=`r summary(services_aic)$r.squared` , p `r lm_pvalue(services_aic)`), with significant main effects of degree and age at diagnosis. Less severe hearing loss `r beta_output(services_aic, 2)` and later diagnosis `r beta_output(services_aic, 4)`, and coming from a non-English-speaking household `r beta_output(services_aic, 5)` predicted later intervention. Prematurity and language background were not significant predictors on their own, but their inclusion improved model fit.
<!-- eb2ec: you left out prematurity in the penultimate sentence, though cf previous comment -->

```{r intervention-betas, fig.cap="Coefficients for intervention model."}
...
<!-- eb2ec: there's still a good bit of work to do in the discussion. There are lots of interesting things you're raising but lots of extra stuff to wittle and shed. I'd like to see an outline version of the discussion (text file or googledoc) where you have the headings and subheadings with the key point each paragraph is making as a single bulleted sentence. rather than doing big edits at this stage i'm going to make some organizational comments and notes of what i think some of the interesting ideas below are -->
# Discussion
...
<!-- eb2ec: first 1-2 para of discussion needs to summarize the key findings for your reader. tie this back to the 3 questions frmo intro (which map onto the 3 parts of your results) -->

## How are child-level variables intertwined?
<!-- eb2ec: i'm not sure what you want the reader to take as the ansewr to this question -->
...
<!-- eb2ec: generally speaking there shouldn't be too many new stats introduced in the discussion that weren't already discussed above unless you're making a new subtle point or summarizing. but if youwant to include cramer's v above, that shoudl be autogenerated -->
...
<!-- eb2ec: this para seems like stuff you said in intro + some  descriptive seeming methods/results stuff along with some vague stuff 'appears probably' ? zero in on your point-->
...
<!-- eb2ec:  this is an interesting point about total comm interp. can you give us a sense of quantification for 'much more likely'? 2x?10x?100x? -->

Additionally, in our sample, children with developmental delays were considerably more likely to receive >10 services per month, perhaps in response to increased need (or increased perceived need). The services per month variable also includes occupational therapy, physical therapy, which typically-developing DHH children may be unlikely to receive. Likewise, children who used total communication were more likely to receive frequent services.
<!-- eb2ec: same question as above  wrt quantity: what's considerable mean, and also something missing in penultimate sentence -->
...
<!-- eb2ec:  not sure what this para adds. is this surprising? consistent with lit? is your possible explanation actually just a health fact about how conductive hearing loss works, or somethig more? -->
...
<!-- eb2ec: the preceding few para's feeling a bit like: result + possible explanation in a bit of a laundry list-y way. we need a through-line -->
...
<!-- eb2ec: THIS is an interesting point! -->

## Predicting vocabulary outcomes
<!-- eb2ec: transition please. this section needs some connecting of the dots across para's: more clear transitions. Small point: i might suggest adding a little code chunk above where you create these #s as vars, e.g.
prop_below_50th <-  ((nrow(elssp_curves %>% filter(diff_age_from_expected > 0))) / (nrow(elssp_curves %>% filter(diff_age_from_expected != '')))) * 100
and then "r prop_below_50th% of DHH children"
for readability's sake. Also why would there be any rows where diff_age_from_expected is ''??? -->

`r ((nrow(elssp_curves %>% filter(diff_age_from_expected > 0))) / (nrow(elssp_curves %>% filter(diff_age_from_expected != '')))) * 100`% of DHH children in our sample fell below the 50th percentile for spoken vocabulary. Of the `r ((nrow(elssp_curves %>% filter(diff_age_from_expected <= 0))) / (nrow(elssp_curves %>% filter(diff_age_from_expected != '')))) * 100`% who were at or above the 50th percentile, `r nrow(elssp_curves %>% filter(diff_age_from_expected < 0 & Age > 9))/nrow(elssp_curves %>% filter(diff_age_from_expected < 0))*100`% were 8-to-9-month olds who were not yet producing any words (as expected at this age). To have such a strong of majority DHH children below the 50th percentile for vocabulary development indicates that this group is not yet well-equipped to acquire spoken language. This disadvantage can have lasting consequences in the lives of DHH children [@kyle2010; @qi2012; @karchmer2003]. 

In contrast to our predictions, the best model predicting vocabulary delay had just a few variables: age, amplification, and laterality. 
...
This could be explained in part by constraints on the variable.--> 
<!-- While age is intrinsically constrained (e.g. 9-month-old cannot be 12 months delayed, but a 24-month-old can),  -->
Notably, we did not simply find that DHH children were learning words at the same rate (albeit delayed) as hearing children, which would have led to a constain delay across developmental time. Instead, we see that the spoken vocabulary delay widens with age, indicating that the rate of spoken vocabulary acquisition is slower for DHH children. The result is a population increasingly behind on spoken language milestones, and given that none of the children here use sign language, on language development more broadly. 
<!-- eb2ec: you get to the interesting part at the end of the para. i did some editing to give you a sense of how i'd do the wittling in this case -->
...
<!-- eb2ec: link between the possible explanation and the evidence for or against it more clearly for your reader, whether that's numerical or rhetorical. -->
...
<!-- eb2ec: i think we have to do this part carefully, right now it feels like the 'so given all our data without sign, we think families should all be learning sign'. as you know it's a super hot button topic. I think i'd suggest turning around the order. Something like: so what else could we be doing to help DHH families build their children's language skills? As noted, this sample didn't learn sign, but it's worth considering bc xyz. Thus, we would recommend ABC. i also added a 'softening' sentence at the beginning of this section, but there's some reworking to do  -->

# Limitations and Opportunities for Future Work
<!-- eb2ec: need clearer scaffolding for this section. youe' telling us about 3 potential limitations/opportunities. what are they? -->
These analyses were exploratory, and there were many possible analytic routes. That said, our results largely converge with or replicate key aspects of past studies [e.g., @ching2013] and received wisdom among clinicians. In the interest of transparency, these data and all code generating our results are available on our OSF page (XXX) and we encourage those interested to explore further analyses. Additionally, many children in our sample have multiple evaluations, so we have an opportunity to see whether these results hold up longitudinally. 
<!-- eb2ec: not sure what the last sentence gets you -->

This sample is comprised only of children in North Carolina, and certain factors, such as diagnosis and early intervention practices, vary by country and by state [@nad]. However, based on Gallaudet's 2014 Annual Survey of Deaf and Hard-of-Hearing Children and the SEELS report of elementary and middle school aged DHH children [@blackorby2006; @gallaudetresearchinstitute2014], our sample largely resembles the national DHH population in terms of degree of hearing loss, percentage of children with additional disabilities, cochlear implant and hearing aid use, and gender. Descriptively, compared to the Gallaudet sample, more children in our sample spoke Spanish, and many fewer children in our sample used sign language. 
<!-- eb2ec; some #s on this? more? many fewer? -->
We would exercise caution in applying these results to regions where sign language access for DHH children is more common (e.g. Washington D.C.; Rochester, New York.) A similar naturalistic study in those regions could help illuminate the effects of different clinical and demographic factors in a signing population
...
<!-- eb2ec: some wittling needed in previous para's -->
...
<!-- eb2ec: this isn't quite a conclusion paragraph, it's more like what the opening of the discussion might contain (but needs a bit of smoothing there too to link to 1/2/3 from intro/results). Conclusion is for the big picture takehome you want to leave them with! -->
