...
As noted above, the CDI is composed of two instruments, which differ in number of questions (i.e. the maximum vocabulary score is 398 on Words and Gestures and 680 on Words and Sentences). To take this into account, rather than using the raw number of words produced as our outcome variable, we use the difference (in months) between the child’s chronological age and their predicted age based on their vocabulary from the WordBank norms (@frank2017). We call this derived variable **vocabulary delay**. 
...
WG_eng_curves <- ggplot()  + 
  ...
  geom_line(data = (quantreg_output%>%filter(form=='WG'&language=='English (American)')), aes(x=age, y=production), colour='black', linetype='dashed')+ 
  theme_classic() + 
...
WS_eng_curves <- ggplot()  + 
  ...
  geom_line(data = (quantreg_output%>%filter(form=='WS'&language=='English (American)')), aes(x=age, y=production), colour='black', linetype='dashed')+ 
  theme_classic() + 
...
WG_span_curves <- ggplot()  + 
  ...
  geom_line(data = (quantreg_output%>%filter(form=='WG'&language=='Spanish (Mexican)')), aes(x=age, y=production), colour='black', linetype='dashed')+ 
  theme_classic() + 
...
WS_span_curves <- ggplot()  + 
  ...
  geom_line(data = (quantreg_output%>%filter(form=='WS'&language=='Spanish (Mexican)')), aes(x=age, y=production), colour='black', linetype='dashed') + 
  theme_classic() + 
...
We performed stepwise model comparison using stepAIC (MASS) to pare down the model. This process selected only the predictors which incrementally improved model fit, measured by Akaike’s Information Criterion (AIC), which considers goodness of fit and model complexity (penalizing models with many predictors). Based on this iterative process, we removed Prematurity, Communication, Meets 1-3-6, and ServicesReceivedPerMonth from the model. 
<!-- eb2ec: why remove language background? do you mean you ran the models separately for spanish and english? or  because they were on the same scale it was just one model, right?
(also: clarify when you introduce CDI: does spanish  W&G and W&S have the same # of items? 
...
full_model_delay <- lm(diff_age_from_expected ~ Gender + DevelopmentalConcerns + IsPremature + HealthIssues + Laterality + HLworse + Amplification + Communication + Meets136 + PrimaryLanguage, data = (elssp_curves %>% filter(Meets136!=''&HLworse!=''&PrimaryLanguage!='Hindi')))
stepAIC(full_model_delay)
#with Spanish -- just Meets136, DevelopmentalConcerns, and Amplification (not sig). R2(adj)=35(31), p<.01
#without Spanish -- Gender(not sig), DevConcerns, Amplification(not sig), 136. R2(adj)=32(25), p=.001

# need to redo this with chosen model
...
data = (elssp_curves %>%  filter(PrimaryLanguage == "English")))

vif(AIC_model_delay)
```


Thus, our final model was: Vocabulary Delay ~ Gender + Developmental Delay + Health Issues + Laterality + Degree + Amplification. This model accounted for significant variance in children's vocabulary delay (adjusted-R^2^ = `r summary(AIC_model_delay)$adj.r.squared`, *p* = `r AIC_model_delay$p.value`). We found a significant main effects for each predictor. In this model, being male `r beta_output(AIC_model_delay, 2)`, having a developmental delay `r beta_output(AIC_model_delay, 3)`, bilateral hearing loss `r beta_output(AIC_model_delay, 5)`, and more severe hearing loss predicted a larger delay `r beta_output(AIC_model_delay, 6)`. Having a cochlear implant `r beta_output(AIC_model_delay, 7)`or hearing aid `r beta_output(AIC_model_delay, 8)` predicted a smaller delay, relative to no amplification. Presence of health issues indicated a smaller vocabulary delay `r beta_output(AIC_model_delay, 4)`.  Although we showed in Part I that relationships exist among several of these variables (e.g., degree and amplification), the vif (variance inflation factor) for our model revealed low levels of collinearity among our predictors (all GVIF < 3; see table \@ref(tab:delay-betas); @james2013).
<!-- eb2ec: something is not right in the preceding para, see rendered text -->
<!-- eb2ec: help your reader understand  your Betas. are they properly interpreted as months of delay so boys are 1.8 months more delayed relative to girls? -->
<!-- eb2ec: what's gvif? -->
...
HLworse = "Degree of Hearing Loss",
...
Perhaps of greatest importance to clinicians and policymakers is the implementation and effect of existing policies. Although whether a child met 1-3-6 guidelines was not in our final model predicting vocabulary delay, its importance for practice and policy merit further discussion regarding. To this end, we looked at the ages at which children received diagnosis and intervention, and how this mapped onto the 1-3-6 guidelines.  In this section, we provide a brief description of the implementation of 1-3-6 in our sample and describe the results of exploratory linear regression models for age at diagnosis and age at intervention. 
...
For age at diagnosis, we included the set of child-specific factors that would be relevant *before* diagnosis of hearing loss (e.g., we excluded amplification type because a child would not receive a hearing aid or cochlear implant prior to being diagnosed with hearing loss.) We began with: gender, degree, developmental delay, health issues, prematurity, laterality, language background, and etiology. Under the best fit model (R^2^=`r summary(diagnosis_aic)$r.squared` , p=`r lm_pvalue(diagnosis_aic)`), having no additional health issues `r beta_output(diagnosis_aic, 2)`, coming from an English-speaking household `r beta_output(diagnosis_aic, 3)`, and having unilateral hearing loss `r beta_output(diagnosis_aic, 4)` predicted earlier diagnosis. This model accounted for roughly `r (summary(diagnosis_aic)$r.squared)*100`% of the variance in age at diagnosis.
...
We repeated this model selection process for age at intervention. In addition to the variables used to fit the intervention model, we included age at diagnosis. The best fit model was Age Intervention ~ prematurity + degree + age at diagnosis + language background (R^2^=`r summary(services_aic)$r.squared` , p=`r lm_pvalue(services_aic)`). Prematurity `r beta_output(services_aic, 3)`, less severe hearing loss `r beta_output(services_aic, 2)`, later diagnosis `r beta_output(services_aic, 4)`, and coming from a non-English-speaking household `r beta_output(services_aic, 5)` predicted later intervention and accounted for roughly `r (summary(services_aic)$r.squared) * 100`% of the variance in age at intervention. 
<!-- eb2ec: same comments apply as above. -->
