...
author: 
  - name          : "Erin Campbell"
    ...
    address       : "508B Gurley St., Durham, NC, 27701"
...
```{r load_packages, cache = FALSE, include = FALSE, echo=FALSE}
library(papaja) 
library(tidyverse)
library(knitr)
library(lubridate)
...
# Methods
``` {r demo-data}
num_subs = nrow(full_elssp)
num_repeat_subs = nrow(full_elssp %>% filter(VisitTotal > 1)) 
num_girls = nrow(full_elssp %>% filter(Gender == 'female'))
num_boys = nrow(full_elssp %>% filter(Gender == 'male'))
min_age = min(full_elssp$Age)
max_age = max(full_elssp$Age)
mean_age = mean(full_elssp$Age)
sd_age = sd(full_elssp$Age)
num_too_old_wg = nrow(wg_elssp %>% filter(Age>18))
num_spanish_cdi = nrow(full_elssp %>% filter(PrimaryLanguage == 'Spanish'))
```
...
num_NICU <- num_downsyndrome <- nrow(comorbid %>% filter(NICU==1))
...
num_illness <- nrow(comorbid %>% filter(chroniclungdisease==1|MRSA==1|meningitis==1|frequentfevers==1|Sepsis==1|jaundice==1|pneumonia==1))
...
num_health_issues_other <- nrow(comorbid %>% filter(esotropia==1|iriscoloboma==1|dysplastickidney==1|CHARGEorVATER==1|apnea==1|hydronephrosis==1|seizures==1|laryngomalacia==1|tonguetie==1|midlinedefect==1|hypoglycemia==1|anemia==1|mitochondrialdisorder==1|skinbreakdown==1|VACTERL==1|analatresia==1|renalabnormalities==1|enlargedadenoids==1|liptie==1|sacraldimple==1))
...
# comorbid_table %>% gt()  %>%
#   tab_header(
#     title = glue::glue("Additional Diagnoses (n={num_anycomorbid})")
#   ) 


kable(comorbid_table,
...
n_HA_u <- nrow(full_elssp %>% filter(Laterality=='Unilateral'&Amplification=='HA'))
n_HA_b <- nrow(full_elssp %>% filter(Laterality=='Bilateral'&Amplification=='HA'))
n_CI_u <- nrow(full_elssp %>% filter(Laterality=='Unilateral'&Amplification=='CI'))
n_CI_b <- nrow(full_elssp %>% filter(Laterality=='Bilateral'&Amplification=='CI'))
n_none_u <- nrow(full_elssp %>% filter(Laterality=='Unilateral'&Amplification=='none'))
n_none_b <- nrow(full_elssp %>% filter(Laterality=='Bilateral'&Amplification=='none'))
n_total_u <- nrow(full_elssp %>% filter(Laterality=='Unilateral'))
n_total_b <- nrow(full_elssp %>% filter(Laterality=='Bilateral'))

meanbetter_HA_u <- mean((full_elssp %>% filter(Laterality=='Unilateral'&Amplification=='HA'))$HLbetter)
meanbetter_HA_b <- mean((full_elssp %>% filter(Laterality=='Bilateral'&Amplification=='HA'))$HLbetter, na.rm = TRUE)
meanbetter_CI_u <- mean((full_elssp %>% filter(Laterality=='Unilateral'&Amplification=='CI'))$HLbetter)
meanbetter_CI_b <- mean((full_elssp %>% filter(Laterality=='Bilateral'&Amplification=='CI'))$HLbetter)
meanbetter_none_u <- mean((full_elssp %>% filter(Laterality=='Unilateral'&Amplification=='none'))$HLbetter)
meanbetter_none_b <- mean((full_elssp %>% filter(Laterality=='Bilateral'&Amplification=='none'))$HLbetter)
meanbetter_total_u <- mean((full_elssp %>% filter(Laterality=='Unilateral'))$HLbetter)
meanbetter_total_b <- mean((full_elssp %>% filter(Laterality=='Bilateral'))$HLbetter, na.rm = TRUE)

meanworse_HA_u <- mean((full_elssp %>% filter(Laterality=='Unilateral'&Amplification=='HA'))$HLworse, na.rm = TRUE)
meanworse_HA_b <- mean((full_elssp %>% filter(Laterality=='Bilateral'&Amplification=='HA'))$HLworse, na.rm = TRUE)
meanworse_CI_u <- mean((full_elssp %>% filter(Laterality=='Unilateral'&Amplification=='CI'))$HLworse)
meanworse_CI_b <- mean((full_elssp %>% filter(Laterality=='Bilateral'&Amplification=='CI'))$HLworse)
meanworse_none_u <- mean((full_elssp %>% filter(Laterality=='Unilateral'&Amplification=='none'))$HLworse, na.rm = TRUE)
meanworse_none_b <- mean((full_elssp %>% filter(Laterality=='Bilateral'&Amplification=='none'))$HLworse)
meanworse_total_u <- mean((full_elssp %>% filter(Laterality=='Unilateral'))$HLworse, na.rm = TRUE)
meanworse_total_b <- mean((full_elssp %>% filter(Laterality=='Bilateral'))$HLworse, na.rm = TRUE)

ageamp_HA_u <- mean((full_elssp %>% filter(Laterality=='Unilateral'&Amplification=='HA'))$AgeAmplification)
ageamp_HA_b <- mean((full_elssp %>% filter(Laterality=='Bilateral'&Amplification=='HA'))$AgeAmplification, na.rm = TRUE)
ageamp_CI_u <- mean((full_elssp %>% filter(Laterality=='Unilateral'&Amplification=='CI'))$AgeImplantation)
ageamp_CI_b <- mean((full_elssp %>% filter(Laterality=='Bilateral'&Amplification=='CI'))$AgeImplantation)
ageamp_total_u <- mean((full_elssp %>% filter(Laterality=='Unilateral'))$AgeAmplification, na.rm = TRUE)
ageamp_total_b <- mean((full_elssp %>% filter(Laterality=='Bilateral'))$AgeAmplification, na.rm = TRUE)

n_HA <- nrow(full_elssp %>% filter(Amplification=='HA'))
n_CI <- nrow(full_elssp %>% filter(Amplification=='CI'))
n_none <- nrow(full_elssp %>% filter(Amplification=='none'))
...
hearing_table <- tribble(
  ...
  glue('NA'),
  glue('Total (n={nrow(full_elssp)})'), glue('{n_total_u} / {n_total_b}'), glue('{round(meanbetter_total_u, digits=2)} / {round(meanbetter_total_b, digits=2)} dB'), glue('{round(meanworse_total_u, digits=2)} / {round(meanworse_total_b, digits=2)} dB'),
  glue('NA')
)
...
```{r comm_info}
n_spoken <- nrow(full_elssp %>% filter(Communication=='spoken'))
n_CS <- nrow(full_elssp %>% filter(Communication=='cued speech'))
n_TC <- nrow(full_elssp %>% filter(Communication=='total communication'))
n_Eng <- nrow(full_elssp %>% filter(PrimaryLanguage=='English'))
n_Span <- nrow(full_elssp %>% filter(PrimaryLanguage=='Spanish'))
n_Hindi <- nrow(full_elssp %>% filter(PrimaryLanguage=='Hindi'))

n_spoken_English <- nrow(full_elssp %>% filter(Communication=='spoken'&PrimaryLanguage=='English'))
n_spoken_Spanish <- nrow(full_elssp %>% filter(Communication=='spoken'&PrimaryLanguage=='Spanish'))
n_spoken_Hindi <- nrow(full_elssp %>% filter(Communication=='spoken'&PrimaryLanguage=='Hindi'))

n_TC_English <- nrow(full_elssp %>% filter(Communication=='total communication'&PrimaryLanguage=='English'))
n_TC_Spanish <- nrow(full_elssp %>% filter(Communication=='total communication'&PrimaryLanguage=='Spanish'))
n_TC_Hindi <- nrow(full_elssp %>% filter(Communication=='total communication'&PrimaryLanguage=='Hindi'))

n_CS_English <- nrow(full_elssp %>% filter(Communication=='cued speech'&PrimaryLanguage=='English'))
n_CS_Spanish <- nrow(full_elssp %>% filter(Communication=='cued speech'&PrimaryLanguage=='Spanish'))
n_CS_Hindi <- nrow(full_elssp %>% filter(Communication=='cued speech'&PrimaryLanguage=='Hindi'))
...
Because of the sparse data on screening age, if participants had an age at diagnosis  <= 3 mo. and an age of intervention  <= 6 mo., they were recorded as meeting 1-3-6. It is possible that a participant did not receive screening by 1 month, but did receive diagnosis by 3 months and services by 6 months. This special case would be coded as meeting 1-3-6 by our criteria.

```{r meets136_info}
# create "Table with meets136 info"
perc_136 <- (nrow(full_elssp%>%filter(Meets136=='yes'))/(nrow(full_elssp %>% filter(Meets136=='yes'|Meets136=='no'))))*100
perc_13 <- (nrow(full_elssp%>%filter(meets13=='yes'))/(nrow(full_elssp %>% filter(meets13=='yes'|meets13=='no'))))*100
perc_6 <- (nrow(full_elssp%>%filter(meets6=='yes'))/(nrow(full_elssp %>% filter(meets6=='yes'|meets6=='no'))))*100

age_diagnosis <- mean(full_elssp$IdentificationOfHLMonths, na.rm = TRUE)
age_diagnosis_sd <- sd(full_elssp$IdentificationOfHLMonths, na.rm = TRUE)

age_services <- mean(full_elssp$AgeStartedServices, na.rm = TRUE)
age_services_sd <- sd(full_elssp$AgeStartedServices, na.rm = TRUE)
...
'Age', 'Continuous', glue('{min(full_elssp$Age)}-{max(full_elssp$Age)} months'),
'Age at Amplification', 'Continuous', glue('{min(full_elssp$AgeAmplification, na.rm=TRUE)}-{max(full_elssp$AgeAmplification, na.rm=TRUE)} months'),
'Age at Diagnosis', 'Continuous', glue('{min(full_elssp$IdentificationOfHLMonths, na.rm=TRUE)}-{max(full_elssp$IdentificationOfHLMonths, na.rm=TRUE)} months'),
'Age at Implantation', 'Continuous', glue('{min(full_elssp$AgeImplantation, na.rm=TRUE)}-{max(full_elssp$AgeImplantation, na.rm=TRUE)} months'),
'Age at Intervention', 'Continuous', glue('{min(full_elssp$AgeStartedServices, na.rm=TRUE)}-{max(full_elssp$AgeStartedServices, na.rm=TRUE)} months'),
...
'Degree Hearing Loss (worse ear)', 'Continuous', glue('{min(full_elssp$HLworse, na.rm=TRUE)}-{max(full_elssp$HLworse, na.rm=TRUE)} dB HL'),
...
'Services Received Per Month', 'Continuous', glue('{min(full_elssp$ServicesReceivedPerMonth, na.rm=TRUE)}-{max(full_elssp$ServicesReceivedPerMonth, na.rm=TRUE)} services / month'),
'Type of Hearing Loss', 'Categorical', 'Sensorineural / Conductive / Mixed',
'CDI - Words Produced', 'Continuous', glue('{min(full_elssp$ProductionCDI)}-{max(full_elssp$ProductionCDI)} words'),
)
...
Shapiro–Wilk tests revealed that all of our continuous measures (i.e. degree of hearing loss, services received per month, vocabulary delay) significantly differed from a normal distribution (ps <.05), so we used nonparametric tests to explore relationships among our variables. For categorical-categorical relationships, we used chi square tests; for continuous-categorical tests, we used mann-whitney U tests (2 levels for categorical variable) or kruskal-wallis tests (>2 levels for categorical variable; for continuous-continuous relationships, we used Of the fifty-five combinations of variables, p < .05 for sixteen, and seven survived bonferroni correction (p < `r round(.05/55, digits = 5)`). The full set of comparisons is shown in figure XXX.


```{r relationships_plot}
print(hm_test_plot)
print(hm_test_plot2)
```

From this analysis, we found that children born premature were more likely to also have health issues (X2 (`r (chisq.test(HealthIssues_Premature))$parameter`, N = `r sum(((chisq.test(HealthIssues_Premature))$observed))`) = `r (chisq.test(HealthIssues_Premature))$statistic`, p = `r (chisq.test(HealthIssues_Premature))$p.value`.) Children with conductive hearing loss were more likely to have unilateral hearing loss (X2 (`r (chisq.test(Etiology_Laterality))$parameter`, N = `r sum(((chisq.test(Etiology_Laterality))$observed))`) = `r (chisq.test(Etiology_Laterality))$statistic`, p = `r (chisq.test(Etiology_Laterality))$p.value`). Children with unilateral hearing loss were unlikely to receive a cochlear implant and more likely to use no amplification (X2 (`r (chisq.test(Amplification_Laterality))$parameter`, N = `r sum(((chisq.test(Amplification_Laterality))$observed))`) = `r (chisq.test(Amplification_Laterality))$statistic`, p = `r (chisq.test(Amplification_Laterality))$p.value`). Children with more severe hearing loss were more likely to use a cochlear implant than children with milder hearing loss (H(`r Degree_Amplification$parameter`)=`r Degree_Amplification$statistic`, p=`r Degree_Amplification$p.value`). Children with developmental delays received more services per month than typically developing DHH children (H(`r Services_DevDelay$parameter`)=`r Services_DevDelay$statistic`, p=`r Services_DevDelay$p.value`)and were more likely to use total communication (X2 (`r (chisq.test(DevConcerns_Communication))$parameter`, N = `r sum(((chisq.test(DevConcerns_Communication))$observed))`)) = `r (chisq.test(DevConcerns_Communication))$statistic`, p = `r (chisq.test(DevConcerns_Communication))$statistic`). Children who used total communication received more services per month than children using spoken language (H(`r Services_Communication$parameter`)=`r Services_Communication$statistic`, p=`r Services_Communication$p.value`).
...
```{r wg_ws_rainbow_plot}
cowplot::plot_grid(wg_rainbow, ws_rainbow)
# caption: Figures XXX - Colored lines show vocabulary percentiles from wordbank. black line shows growth curve from 50th percentile data. Black triangles show vocabulary scores of individual DHH children.
```
...
perc_136_TD <- (nrow(full_elssp%>%filter(DevelopmentalConcerns=='no')%>%filter(Meets136=='yes'))/(nrow(full_elssp%>%filter(DevelopmentalConcerns=='no')%>%filter(Meets136=='yes'|Meets136=='no'))))*100
perc_136_DA <- (nrow(full_elssp%>%filter(DevelopmentalConcerns=='yes')%>%filter(Meets136=='yes'))/(nrow(full_elssp%>%filter(DevelopmentalConcerns=='yes')%>%filter(Meets136=='yes'|Meets136=='no'))))*100
```
Lastly, we looked at the ages at which children received diagnosis and intervention, and how this mapped onto the 1-3-6 guidelines. `r perc_136`% of our sample met 1-3-6 guidelines for early diagnosis and intervention. Of children with comorbidities (developmental concerns, prematurity, health issues), only `r round(perc_136_DA, digits = 0)`% met 1-3-6 guidelines, compared to `r perc_136_TD`% of typically developing children. See table XXX for complete breakdown.
...
#caption: Figure XXX - Dots represent average age of diagnosis, etc. Stronger lines represent standard deviation. Softer lines show range. 
```
...
The best fit model (R2=`r summary(diagnosis_aic)$r.squared` , p=`r lm_pvalue(diagnosis_aic)` )included health issues (beta weight = `r summary(diagnosis_aic)$coefficients[2,1]`, p = `r summary(diagnosis_aic)$coefficients[2,4]`) and language background (beta weight = `r summary(diagnosis_aic)$coefficients[3,1]`, p = `r summary(diagnosis_aic)$coefficients[3,4]`).
Age diagnosis ~ health issues + language background

```{r diagnosis_betas}
diagnosis_aic <- lm(formula = IdentificationOfHLMonths ~ HealthIssues + Monolingual_English, data = full_elssp)
...
The best fit model (R2=`r summary(services_aic)$r.squared` , p=`r lm_pvalue(services_aic)`) included prematurity (beta weight = `r summary(services_aic)$coefficients[4,1]`, p = `r summary(services_aic)$coefficients[4,4]`), laterality (beta weight = `r summary(services_aic)$coefficients[3,1]`, p = `r summary(services_aic)$coefficients[3,4]`), degree of hearing loss (beta weight = `r summary(diagnosis_aic)$coefficients[2,1]`, p = `r summary(services_aic)$coefficients[2,4]`), and age at diagnosis (beta weight = `r summary(services_aic)$coefficients[5,1]`, p = `r summary(services_aic)$coefficients[5,4]`). 
Age intervention ~ laterality + degree (worse ear) + prematurity + laterality + age diagnosis

```{r intervention_betas}
services_aic <- lm(formula = AgeStartedServices ~ HLworse + Laterality + IsPremature + IdentificationOfHLMonths, data = (data = full_elssp %>% filter(HLworse != "NA")))
