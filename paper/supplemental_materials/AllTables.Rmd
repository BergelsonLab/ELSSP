...
as.data.frame(tribble(
  ...
  flextable() %>%
  add_header_lines(values = "Table 1: *Variables List: Detailed information about the variables studied.*") %>%
  add_footer_lines("*Note.* For categorical variables, levels are described. Some participants had missing information for some variables, thus totals may not sum to 100. For continuous variables, range, mean, and standard deviation are provided. For CDI, participants were *either* administered Words and Gestures *or* Words and Sentences*.") %>%
  ft_theme_apa() %>%
  FitFlextableToPage() %>% 
  save_as_docx(path= "paper/tables/Table1.docx")
  
  
  # kable(caption = " For categorical variables, levels are described. For continuous variables, range, mean, and standard deviation are provided.") %>%
  # # kable_styling(latex_options = c("scale_down")) %>%
  # kable_styling(font_size = 6)

# Caption: "Detailed information about the variables studied. 
...
tribble(
  ...
  as.data.frame() %>%
  flextable() %>%
  add_header_lines(values = "Table S1: *Additional Diagnoses (n = 39): Ns of participants in our sample diagnosed with other conditions.*") %>%
  add_footer_lines("*Note.* Ns do not sum to total because many participants had multiple diagnoses.") %>%
  ft_theme_apa() %>%
  FitFlextableToPage() %>% 
  save_as_docx(path= "paper/supplemental_materials/TableS1.docx")
...
elssp %>%
  ...
  as.data.frame() %>%
  flextable() %>%
  add_header_lines(values = "Table S2: *Audiological Characteristics of the Sample*") %>%
  add_footer_lines("*Note.* First two columns describe laterality and amplification type (cochlear implant (CI), hearing aid (HA), or none). Mean decibels of hearing loss (HL) in better ear, worse ear, and the mean age (in months) of amplification, and cochlear implantation (when applicable) for each laterality and amplification combination.") %>%
  ft_theme_apa() %>%
  FitFlextableToPage() %>% 
  save_as_docx(path= "paper/supplemental_materials/TableS2.docx")
...
elssp %>%
  ...
  as.data.frame() %>%
  flextable() %>%
  add_header_lines(values = "Table S3: *Language and Communication Characteristics of the Sample: Ns of participants by language background and communication method.*") %>%
  ft_theme_apa() %>%
  FitFlextableToPage() %>% 
  save_as_docx(path= "paper/supplemental_materials/TableS3.docx")
...
AIC_model_delay %>%
  ...
  as.data.frame() %>%
  flextable() %>%
  add_header_lines(values = "Table S4: *Unstandardized Beta Weights (Months of Vocabulary Delay) for the Model of Vocabulary Delay Selected by AIC.*") %>%
  ft_theme_apa() %>%
  FitFlextableToPage() %>% 
  save_as_docx(path= "paper/supplemental_materials/TableS4.docx")
...
diagnosis_aic %>%
  ...
  as.data.frame() %>%
  flextable() %>%
  add_header_lines(values = "Table S5: *Unstandardized Beta Weights (Months) for the Model of Age at Diagnosis Selected by AIC.*") %>%
  ft_theme_apa() %>%
  FitFlextableToPage() %>% 
  save_as_docx(path= "paper/supplemental_materials/TableS5.docx")
...
services_aic %>%
  ...
  as.data.frame() %>%
  flextable() %>%
  add_header_lines(values = "Table S6: *Unstandardized Beta Weights (Months) for the Model of Age at Diagnosis Intervention by AIC.*") %>%
  ft_theme_apa() %>%
  FitFlextableToPage() %>% 
  save_as_docx(path= "paper/supplemental_materials/TableS6.docx")
