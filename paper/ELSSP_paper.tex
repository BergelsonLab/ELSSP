...
\caption{\label{tab:CDIinfo}CDI details: For each version, average age, comprehension score, production score of participants in our sample (with standard deviations in parentheses). Final column shows the percentage of children administered the version who were diagnosed with developmental delays.}
\centering
\resizebox{\linewidth}{!}{
...
\caption{\label{tab:amp-info}Audiological Characteristics of the Sample: First two columns describe laterality and amplification type (cochlear implant, hearing aid, or none). Next two columns show mean decibels of hearing loss in better/worse ear for the subset of the population given that combination of laterality and amplification. Last two columns describe age (in months) at amplification or cochlear implantation (if applicable) for the laterality-amplification subset.}
\centering
\resizebox{\linewidth}{!}{
...
\caption{\label{fig:meets136-timeline}Violin plots for age at diagnosis, intervention, amplification, and cochlear implantation. Each dot represents the age that one child received the clinical service. Blacks dot and whiskers show means and standard deviations.}
\end{figure}

\begin{figure}
\centering
\includegraphics{ELSSP_paper_files/figure-latex/delay-violins-1.pdf}
\caption{\label{fig:delay-violins}Violin plots showing vocabulary delay for children who meet 1-3-6 guidelines for diagnosis/intervention (top violin) and children who do not (bottom violin). Each dot represents one child in the sample. Blacks dot and whiskers show means and standard error.}
\end{figure}
...
\caption{\label{tab:diagosis-vif}Variance Inflation Factor for diagnosis model: Vocabulary Delay ~ Health Issues + Language Background + Laterality.}
\centering
\begin{tabular}[t]{l|r|r}
\hline
  & VIF & Df\\
